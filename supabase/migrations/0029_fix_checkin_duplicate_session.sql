-- ============================================================================
-- 0029_fix_checkin_duplicate_session.sql
-- Fixes the duplicate key violation by ensuring session_number calculation
-- considers all existing records for the day, not just completed ones.
-- ============================================================================
...
begin
  ...
  -- Now check for any remaining active session
  if exists (
  ...
  -- Find the highest session number used today, regardless of state.
  ...
  if v_max_session_num is null then
    ...
    -- If we are here, it means there are existing sessions.
    -- We only allow a new session if multiple sessions are enabled OR if there is admin approval.
    if v_employee.allow_multiple_sessions then
      v_session_number := v_max_session_num + 1;
    else
      -- For single-session employees, we check if they have already completed a session.
      -- If the only session that exists is a failed/missing one, we might allow them to "retry" session 1
      -- but for simplicity and audit trails, we usually require approval for any subsequent attempt.

      -- Check if they've already had a successful checkout today
      if exists (
        select 1 from attendance
        where employee_id = v_employee_id
          and attendance_date = v_today
        ...
        -- they can restart Session 1 without a new approval, as the previous one failed.
        -- We just need to make sure we don't collide.
        -- In this case, we can just use v_max_session_num + 1 to be safe.
        v_session_number := v_max_session_num + 1;
      end if;
    end if;
  end if;
