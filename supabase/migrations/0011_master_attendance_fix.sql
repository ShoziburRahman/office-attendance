-- ============================================================================
-- 0011_master_attendance_fix.sql
-- Consolidated fix for check-in/out logic to resolve "completed session" blocks.
-- ============================================================================
...
begin
  ...
  -- Block ONLY if there is a currently open session (checked in but not checked out)
  if exists (
    ...
    raise exception 'You have an active session. Please check out before checking in again';
  end if;

  -- 4. Multi-session / Same-day Logic
  -- We ONLY count real attendance sessions (OFFICE or WFH).
  -- Synthesized records like ABSENT, PAID_LEAVE, UNPAID_LEAVE are ignored.
  select count(*) into v_completed_sessions
    from attendance
    where employee_id = v_employee_id
      and attendance_date = v_today
      and attendance_type is not null
      and day_classification not in ('ABSENT', 'PAID_LEAVE', 'UNPAID_LEAVE');

  if v_completed_sessions = 0 then
    v_session_number := 1;
  elsif v_employee.allow_multiple_sessions then
    -- Employee has standing permission for multiple sessions per day
    v_session_number := v_completed_sessions + 1;
  else
    -- Look for a specific approval for a second+ session today
  ...
  -- 7. Cleanup automatic records
  -- If the employee is checking in, remove any ABSENT/LEAVE record for today
...
begin
  ...
  if v_minutes_since_checkin < v_settings.min_minutes_before_checkout then
    raise exception 'CHECKOUT_LOCKED:' || v_settings.min_minutes_before_checkout::text;
  end if;
