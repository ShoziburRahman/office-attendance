-- ============================================================================
-- 0024_fix_session_duration.sql
-- Fixes the session duration regression and implements automatic forced closure.
-- ============================================================================

-- 1. Restore the max session duration check in fn_check_out
...
declare
  ...
  v_qr_ok            boolean := true; -- Always true now (per migration 0015)
  v_loc_ok           boolean := false;
  v_dist             numeric;
  v_minutes_since_checkin numeric;
begin
  select * into v_settings from office_settings limit 1;
  select * into v_attendance from attendance where id = p_attendance_id and employee_id = v_employee_id and attendance_state = 'CHECKED_IN';
  if not found then
    raise exception 'Active session not found.';
  end if;

  -- RESTORED: Max session duration check
  v_minutes_since_checkin := extract(epoch from (v_now - v_attendance.check_in_at)) / 60;
  ...
  update attendance set
    total_duration_minutes = extract(epoch from (check_out_at - check_in_at)) / 60,
    working_minutes = extract(epoch from (check_out_at - check_in_at)) / 60,
    regular_minutes = extract(epoch from (check_out_at - check_in_at)) / 60,
    overtime_minutes = 0
  where id = p_attendance_id
  returning * into v_attendance;
...
-- 2. Implement automatic forced closure in fn_check_in
...
begin
  ...
  -- If a session is open and exceeds max_session_minutes, we "close" it as MISSING_CHECKOUT.
  -- This removes it from the uq_attendance_one_active_session index (where check_out_at is null).
  update attendance
